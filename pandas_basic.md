...
XXX df[['인덱스0', '열0']]
XXX df[['열0'], ['열1']]
XXX df[['열0'], ['인덱스0']]	# value를 통한 DataFrame 추출 불가 [  ]
```

> df.loc [[  ]]

```python
df.loc[['인덱스0']]	# 인덱스0 행을 DataFrame 으로 추출
df.loc[['인덱스0', '인덱스2']]	# 인덱스0, 인덱스2 행을 DataFrame 으로 추출
df.loc[['인덱스0'], ['열0']]	 # 인덱스0, 열0 에 교차되는 value로 DataFrame 추출
XXX df.loc[['열0']]
XXX df.loc[['열'], ['인덱스0']]
XXX 슬라이싱
```
...
```python
df.iloc[[0]]		# 0번째 인덱스 행으로 이루어진 DataFrame
df.iloc[[0, 1]]		# 0, 1 번째 인덱스 행으로 이루어진 DataFrame
df.iloc[[0], [1]]	# 0번째 인덱스와 1번째 열에 교차되는 value로 이루어진 DataFrame
```
