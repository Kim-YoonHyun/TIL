...
[tool.setuptools.packages.find]	# 제외 디렉토리 설정
exclude = ["tests*", "docs*", "build*", "__pycache__"]
```
...
## pyproject.toml(setup.py 를 사용하지 않을 경우)

pyproject.toml 파일에 모든 설정을 함
...
license = {text = "MIT"}
authors = [
  {name = "kimyh", email = "kim_yh663927@naver.com"}
]
classifiers = [
  "Programming Language :: Python :: 3",
...
where = ["."]
exclude = ["test", "./upload.py"]

[tool.setuptools.package-data]
"textbasic.basic" = ["dictionary.json"]	# 서브 모듈 안의 JSON 파일 포함
...
```bash
python -m build
# 만약 에러 발생시 pip install build 필요
```
