...
docker-compose.yml 을 생성하고 이를 통해서 docker 실행을 제어하는 방법

한 docker 이미지 내에 실행가능한 복수의 서비스가 있는 경우 활용 가능
...
services:
  ...
  # 공통 설정 불러오기
    ...
    image: <이미지 이름>
    # 이미지 내 소스코드 실행 명령어
    # command: ["python", "-m", "analy_module.run", "--run_mode", "normal", "--env", "release"]
...
- 이후 docker image 사용 방식과 동일

## [docker 컨테이너]

임시로 활용 가능한 컨테이너를 열어서 마음대로 사용할 수 있는 환경을 구성
...
- Docker 파일 설정
  ...
  # 2. 접속용 계정 생성
  RUN useradd -m -s /bin/bash <계정이름> && echo "<계정이름>:<비밀번호>" | chpasswd
...
- 빌드
  ```bash
  docker build -t <이미지 이름> .  # . 있음 주의
  ```

- 컨테이너화 진행
  ```bash
  docker run -d -p <포트번호>:22 --name <컨테이너이름> <이미지이름>
  ```
...
- 컨테이너와 포트 매핑 확인
  ...
  ```bash
  docker ps -f "name=<컨테이너이름>"
  # STATUS 가 Up 인지 확인
  ```

- 구동 여부 확인
  ```bash
  docker exec -it <컨테이너_이름> service ssh status
  # is not running 으로 뜰 경우 수동으로 켜기
  # docker exec -it <컨테이너_이름> service ssh start
  ```

- `포트번호` 에 대한 방화벽 설정, 사용가능 여부 확인

   ```bash
   sudo ufw status | grep <포트번호>
   ```

#### 2-1) host GPU 할당

host GPU 를 할당하기 위해서는 우선 host 에 NVIDIA Container Toolkit 이 설치되어야 한다.
...
- 일반 접속
  ssh 접속 방식과 완전히 동일(vscode, putty 등)

- root 로 접속
  ```bash
  docker exec -u 0 -it <컨테이너이름> /bin/bash
  ```

- 컨테이너 내 계정에 sudo 권한이 안 주어질 경우
  ```bash
  docker exec -u 0 -it <컨테이너이름> /bin/bash -c "echo 'ljs ALL=(ALL:ALL) NOPASSWD:ALL' >> /etc/sudoers"
  ```
...
- GPU 할당 하여 컨테이너 재시작
  ...
  ※ 주의
  위는 기존의 이미지로 다른 컨테이너를 재생성 하는 것일 뿐 기존 container 를 재시작하는 것과는 아예 별개다.
  기존 컨테이너의 내용을 유지하고 싶다면 컨테이너를 이미지로 만든 후 그 이미지로 다른 컨테이너를 여는 것이 최선이다.
...
```bash
# -i 옵션은 파일 형식을 스스로 검사하므로 가장 오류가 적음
