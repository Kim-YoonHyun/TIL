...
index_name = ["인덱스0", "인덱스1", "인덱스2", "인덱스3", "인덱스4", "인덱스8"]
...
columns_choice = ["열0", "열1", "열2", "열3", "열4", "열8"]
...
df = pd.DataFrame(data1, 
                  index=index_name, 		# 인덱스 이름 지정
                  columns=columns_name)	# 사용할 열 선택
...
- 열과 인덱스 이름은 기본적으로 숫자값.
- 인덱스와 열의 이름을 지정할 경우, 갯수는 입력데이터와 일치해야함.
...
df = pd.DataFrame(data2, 
                  index=index_name, 		# 인덱스 이름 지정
                  columns=columns_choice)	# 사용할 열 선택

'''return
      열0   열1   열2   열3    열4   열8
인덱스0  aa  100  400  700  1000  NaN
인덱스1  bb  200  500  800  1100  NaN
인덱스2  cc  300  600  900  1200  NaN
'''
```

- 기본적으로 key 값이 열 이름, 인덱스는 숫자 값.
- 인덱스의 갯수는 입력데이터와 일치해야함
- 사용할 열을 선택할 때 데이터에 없는 이름을 지정할 경우, df 의 value 는 NaN 이 됨.		       

### 2.2.4. series -> dataframe
...
df[df['열2'].notnull()] # 열2 기준 NaN가 존재하지 않는 행으로 df 재구성 
